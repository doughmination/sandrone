...
def test_help_lists_commands_and_nested_group_commands() -> None:
    ...
    assert [command.qualified_name for command in commands] == [
        "alpha",
        "tools beta",
    ]
...
def test_section_key_follows_the_folder_a_command_was_loaded_from() -> (
    None
):
    assert (
        sectionKey(moduleCommand("animal", "commands.cogs.fun.animals"))
        == "fun"
    )
    assert (
        sectionKey(moduleCommand("qr", "commands.cogs.tools.qr"))
        == "tools"
    )
    assert (
        sectionKey(moduleCommand("genshin", "commands.cogs.genshin"))
        == "other"
    )
...
def test_grouped_commands_bucket_by_section_in_display_order() -> None:
    ...
    assert [command.name for command in buckets["fun"]] == [
        "8ball",
        "animal",
    ]
...
def test_help_panel_offers_a_section_per_bucket_plus_overview() -> None:
    ...
    assert [option.default for option in select.options] == [
        True,
        False,
        False,
    ]
...
def test_help_panel_falls_back_to_overview_for_unloaded_section() -> None:
    ...
    assert [option.value for option in selectOf(panel).options] == [
        "overview",
        "fun",
    ]
...
def test_help_panel_drops_the_menu_when_nothing_is_loaded() -> None:
    ...
    assert not any(
        isinstance(child, ui.ActionRow) for child in container.children
    )
