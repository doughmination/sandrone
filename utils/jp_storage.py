...
class Field:
    ...
    def __repr__(self) -> str:
        return (
            f"<Field {self.scope.db.name}:{self.scope.label}.{self.name}>"
        )
    ...
    def ids(self) -> builtins.list[builtins.int]:
        return _asIds(self.scope.data().get(self.name))

    def int(
        self, default: builtins.int | None = None
    ) -> builtins.int | None:
        value = self.scope.data().get(self.name)
        if value is None:
            return default
...
class DataBase:
    ...
    def open(self) -> JPConfig:
        if self._config is None:
            try:
                ...
                print(
                    cf.red(f"[database] could not read {self.path}: {e}")
                )
    ...
    def save(self, *, force: bool = False) -> DataBase:
        ...
        if (
            ...
            and cfg.get(versionKey) != self.version
        ):
